...
// Size of a BC13/14 variable-reference instruction (PUSH/POP with type1 = VARIABLE) after the opcode rewrite.
static uint32_t bytecodeVersion14InstructionSize(uint8_t opcode, uint8_t type1) {
    switch (opcode) {
        case OP_PUSH: case OP_PUSHLOC: case OP_PUSHGLB: case OP_PUSHBLTN:
            if (type1 == GML_TYPE_INT16) return 4;
            if (type1 == GML_TYPE_DOUBLE || type1 == GML_TYPE_INT64) return 12;
            return 8;
        case OP_POP: return (type1 == GML_TYPE_INT16) ? 4 : 8;
        case OP_CALL: return 8;
        case OP_BREAK: return (type1 == GML_TYPE_INT32) ? 8 : 4;
        default: return 4;
    }
}

// Rewrites BC13/14 variable references to match BC16 semantics.
static void rewriteBytecode14To16Variables(VMContext* ctx) {
    DataWin* dw = ctx->dataWin;
    uint8_t* buf = dw->bytecodeBuffer;
    size_t base = dw->bytecodeBufferBase;
    uint32_t originalCount = dw->vari.variableCount;

    repeat(originalCount, varIdx) {
        Variable* v = &dw->vari.variables[varIdx];
        int16_t builtinId = VMBuiltins_resolveBuiltinVarId(v->name);
        if (builtinId != BUILTIN_VAR_UNKNOWN) {
            v->varID = -6;
            v->instanceType = INSTANCE_BUILTIN;
            v->builtinVarId = builtinId;
        } else {
            v->varID = (int32_t) varIdx;
            // We don't know yet if this is a global variable
            v->instanceType = INSTANCE_SELF;
            v->builtinVarId = BUILTIN_VAR_UNKNOWN;
        }
    }

    CodeLocals* newCodeLocals = safeCalloc(dw->code.count, sizeof(CodeLocals));
    // stb_ds Variables hashmap
    Variable* newVars = nullptr;

    repeat(dw->code.count, codeIdx) {
        CodeEntry* entry = &dw->code.entries[codeIdx];
        newCodeLocals[codeIdx].name = entry->name;
        if (entry->length == 0) continue;

        IntIntHashMap varIdxToSlot = {0};
        uint32_t entryBase = originalCount + (uint32_t) arrlen(newVars);
        // stb_ds per-entry local slot table for synthetic CodeLocals
        LocalVar* locals = nullptr;
        ...
        while (end > ip) {
            ...
            uint8_t type1 = (instr >> 16) & 0x0F;
            uint32_t size = bytecodeVersion14InstructionSize(opcode, type1);

            if ((opcode == OP_PUSH || opcode == OP_POP) && type1 == GML_TYPE_VARIABLE) {
                uint32_t operandAddr = instrAddr + 4;
                uint32_t operand = BinaryUtils_readUint32(&buf[operandAddr - base]);

                // 0x40000000u = globalvar in BC13/BC14
                if (operand & 0x40000000u) {
                    // Clear the globalvar
                    operand &= ~0x40000000u;
                    // Rewrite the operand
                    BinaryUtils_writeUint32(&buf[operandAddr - base], operand);
                    // Rewrite the instruction to be a INSTANCE_GLOBAL instanceType
                    instr = (instr & 0xFFFF0000u) | (uint16_t) INSTANCE_GLOBAL;
                    BinaryUtils_writeUint32(&buf[instrAddr - base], instr);
                }

                uint32_t varIdx = operand & 0x07FFFFFFu;
                // This gets the NEW instanceType (that also includes the previously rewritten INSTANCE_GLOBAL!)
                int16_t instanceType = (int16_t) (instr & 0xFFFFu);
                if (originalCount > varIdx) {
                    Variable* origV = &dw->vari.variables[varIdx];
                    if (instanceType == INSTANCE_GLOBAL) {
                        if (origV->varID != -6) {
                            // Not builtin && used globalvar, then let's mark it as a INSTANCE_GLOBAL!
                            origV->instanceType = INSTANCE_GLOBAL;
                        }
                    } else if (instanceType == INSTANCE_LOCAL) {
                        // For instance locals, we synthetize a brand new slot for EACH local variable (we don't share it based on the name!)
                        uint32_t beforeCount = varIdxToSlot.count;
                        uint32_t slot = IntIntHashMap_getOrInsertSequential(&varIdxToSlot, (int32_t) varIdx);
                        uint32_t newIdx = entryBase + slot;
                        if (slot == beforeCount) {
                            Variable nv = {
                                .name = origV->name,
                                .varID = (int32_t) slot,
                                ...
                                .occurrences = 0,
                                .firstAddress = 0,
                                .builtinVarId = BUILTIN_VAR_UNKNOWN,
                            };
                            arrput(newVars, nv);
                            LocalVar lv = { .varID = slot, .name = origV->name };
                            arrput(locals, lv);
                        }
                        uint32_t upperBits = operand & 0xF8000000u;
                        BinaryUtils_writeUint32(&buf[operandAddr - base], upperBits | (newIdx & 0x07FFFFFFu));
                    }
                }
            }
            ip += size;
        ...
        // Generate synthetic CodeLocals
        uint32_t entryLocalCount = (uint32_t) arrlen(locals);
        if (entryLocalCount > 0) {
            newCodeLocals[codeIdx].localVarCount = entryLocalCount;
            newCodeLocals[codeIdx].locals = safeMalloc(entryLocalCount * sizeof(LocalVar));
            memcpy(newCodeLocals[codeIdx].locals, locals, entryLocalCount * sizeof(LocalVar));
            entry->localsCount = (uint16_t) entryLocalCount;
            requireMessageFormatted(MAX_CODE_LOCALS > entry->localsCount, "Code %s has too many locals (%u >= MAX_CODE_LOCALS)!", entry->name, entry->localsCount);
        }
        arrfree(locals);
        IntIntHashMap_free(&varIdxToSlot);
    }

    uint32_t newCount = originalCount + (uint32_t) arrlen(newVars);
    if (newCount > originalCount) {
        dw->vari.variables = safeRealloc(dw->vari.variables, newCount * sizeof(Variable));
        memcpy(dw->vari.variables + originalCount, newVars, arrlen(newVars) * sizeof(Variable));
    }
    dw->vari.variableCount = newCount;
    arrfree(newVars);

    if (dw->func.codeLocals) {
        repeat(dw->func.codeLocalsCount, i) free(dw->func.codeLocals[i].locals);
        free(dw->func.codeLocals);
    }
    dw->func.codeLocals = newCodeLocals;
    dw->func.codeLocalsCount = dw->code.count;
}
...
VMContext* VM_create(DataWin* dataWin) {
    ...
    // The reason why this is done here and NOT after rewriteBytecode14To16 is because we depend on the patched varIdxs
    if (dataWin->gen8.bytecodeVersion == 13 || dataWin->gen8.bytecodeVersion == 14) {
        rewriteBytecode14To16Variables(ctx);
    }
